...
const DATA_TYPE_KEYS: { value: string; labelKey: string }[] = [
  { value: 'string', labelKey: 'fields.stringType' },
  { value: 'number', labelKey: 'fields.numberType' },
  { value: 'percent', labelKey: 'fields.percentType' },
  { value: 'money', labelKey: 'fields.moneyType' },
  { value: 'date', labelKey: 'fields.dateType' },
]
...
export default function AnalyticFields({ analyticId }: Props) {
  ...
  const handleAdd = async () => {
    await api.createField(analyticId, { name: t('fields.newField'), data_type: 'string', sort_order: fields.length })
    load()
  }
  ...
  return (
    <Box sx={{ mb: 3, maxWidth: 600 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        <Typography variant="subtitle1">{t('fields.title')}</Typography>
        {!isPeriods && (
            ...
            {fields.map(f => (
              <TableRow key={f.id}>
                ...
                <TableCell>
                  ...
                  >
                    {DATA_TYPE_KEYS.map(dt => <MenuItem key={dt.value} value={dt.value}>{t(dt.labelKey)}</MenuItem>)}
                  </Select>
                </TableCell>
                <TableCell>
                  {!isPeriods && (
